...
+ '<div class="calc-highlight"><strong>Bu derste ne öğreneceksin:</strong> <a href="/tutorials/ml-theory/2">Ders 2</a>\'de sayısal hedef tahmin ettik. Şimdi kategorik hedefe — <em>"bu müşteri kayıp mı?"</em> evet/hayır sorusuna — geçiyoruz. Lojistik regresyonun lineer regresyonun nasıl bir uzantısı olduğunu, sigmoid\'in neyi çözdüğünü, cross-entropy kayıp fonksiyonunu ve karar sınırı kavramını göreceksin. Üstüne kısaca <strong>kNN</strong> ve <strong>Naive Bayes</strong> alternatif algoritmaları tanıtılacak.</div>'
...
+ '<li><strong>Karar ağaçları (<a href="/tutorials/ml-theory/7">Ders 7</a>):</strong> Doğal olarak lineer olmayan, eksen-paralel sınırlar.</li>'
...
+ '<p class="l-text"><strong>Naive Bayes</strong> Bayes teoremi üzerine kuruludur. Detaylı olarak <a href="/tutorials/nlp/3">NLP Ders 3</a>\'te işlenir; burada kısa hatırlatma:</p>'
...
+ '<div class="calc-highlight"><strong>What you will learn:</strong> In <a href="/tutorials/ml-theory/2">Lesson 2</a> we predicted a numeric target. Now we move to a categorical target — <em>"will this customer churn?"</em>, yes/no. You will see how logistic regression extends linear regression, what sigmoid solves, the cross-entropy loss, and the geometric idea of a decision boundary. We will also briefly cover <strong>kNN</strong> and <strong>Naive Bayes</strong> as alternative classifiers.</div>'
...
+ '<li><strong>Decision trees (<a href="/tutorials/ml-theory/7">Lesson 7</a>):</strong> naturally non-linear, axis-aligned boundaries.</li>'
...
+ '<p class="l-text"><strong>Naive Bayes</strong> is built on Bayes\' theorem. Detailed in <a href="/tutorials/nlp/3">NLP Lesson 3</a>; quick reminder:</p>'
...
+ '<p class="l-text"><strong>What this code does:</strong> Three classifiers on the same data. <code>StandardScaler</code> centres each feature to zero mean and unit variance — critical for kNN (distance becomes meaningless if age 28 vs fee 55 dominate). Logistic regression works without scaling but it is good practice. All three predict "churn" for the new customer with different probabilities. Which one is actually best needs cross-validation (<a href="/tutorials/ml-theory/4">Lesson 4</a>).</p>'

+ '<h2 class="l-title">11. What\'s Next</h2>'

+ '<p class="l-text">We have classification fundamentals. But how do we know our predictions are "good"? Are 6 correct training predictions enough, or did the model just memorise? <a href="/tutorials/ml-theory/4">Lesson 4</a> tackles <strong>generalisation</strong>: bias-variance trade-off, train/val/test split, cross-validation. One of the most important ideas in ML.</p>'
