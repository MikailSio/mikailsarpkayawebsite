...
+ '<div class="l-warn"><strong>Sızıntı tehlikesi (<a href="/tutorials/ml-theory/4">L4</a>):</strong> Scaler\'ı sadece <em>train</em> setinde fit et. Test\'i train\'in istatistikleriyle transform et. Pipeline kullanırsan otomatik halleder.</div>'
...
+ '<p class="l-text">Bu durumda model "hep çoğunluğa söyle" stratejisini öğrenir. Accuracy %99.9 görünür ama bir tek dolandırıcılığı bile yakalayamaz. <a href="/tutorials/ml-theory/5">Ders 5</a>\'te gördük; çözüm ne?</p>'
...
+ '<p class="l-text">Modeli olduğu gibi eğit, ama karar eşiğini değiştir. Varsayılan 0.5 yerine 0.3 yaparsan azınlık sınıfı daha sık tahmin edilir. Recall artar, precision düşer. <a href="/tutorials/ml-theory/5">L5</a>\'te gördüğümüz precision-recall trade-off.</p>'
...
+ '<p class="l-text">Modeli ve veriyi en iyi haline getirdik. Şimdi son aşamaya: <em>üretime nasıl çıkarırız?</em> <a href="/tutorials/ml-theory/11">Ders 11</a>: <strong>MLOps & Üretim</strong>. sklearn Pipeline serileştirme, REST API, izleme, model drift, CI/CD, A/B testleri. Eğitilmiş bir modelin gerçek değeri burada yatar.</p>'
...
+ '<div class="l-warn"><strong>Leakage warning (<a href="/tutorials/ml-theory/4">L4</a>):</strong> fit the scaler only on the <em>train</em> set; transform test using train\'s statistics. Pipelines handle this automatically.</div>'
...
+ '<p class="l-text">In such cases the model learns "always predict majority". Accuracy looks like 99.9% but it catches no fraud at all. We saw this in <a href="/tutorials/ml-theory/5">Lesson 5</a>; what to do?</p>'
...
+ '<p class="l-text">Train as usual, but lower the decision threshold. Default 0.5 → 0.3 makes the minority class predicted more often. Recall goes up, precision down — the precision-recall trade-off from <a href="/tutorials/ml-theory/5">L5</a>.</p>'
...
+ '<p class="l-text">We have polished both model and data. Now the final stage: <em>how do we deploy?</em> <a href="/tutorials/ml-theory/11">Lesson 11</a>: <strong>MLOps & Production</strong>. sklearn pipeline serialisation, REST APIs, monitoring, model drift, CI/CD, A/B testing. The bedrock of a trained model\'s real value.</p>'
