...
+ '<p class="l-text"><a href="/tutorials/deep/1">Ders 1</a>\'de bir nöronun şunu yaptığını gördük: <em>z = w<sup>⊤</sup>x + b</em>, sonra <em>ŷ = f(z)</em>. Eğer <em>f</em> sadece özdeşlik (<em>f(z) = z</em>) olsaydı, bütün ağın yaptığı sadece lineer dönüşüm olurdu. Hangi kadar katman üst üste konursa konsun, sonuç hâlâ lineer.</p>'
...
+ '<p class="l-text">Lojistik regresyondan tanıdık (<a href="/tutorials/ml-theory/3">ML L3</a>): herhangi bir reel sayıyı (0, 1) aralığına sıkıştırır. 0\'a yakın için σ ≈ 0.5; büyük pozitif için 1; büyük negatif için 0. İlk sinir ağlarının (1980-2010) varsayılan aktivasyonuydu.</p>'
...
+ '<p class="l-text"><a href="/tutorials/ml-theory/2">ML L2</a>\'de MSE, <a href="/tutorials/ml-theory/3">ML L3</a>\'te cross-entropy gördük. DL\'de aynı temelden başlarız.</p>'
...
+ '<p class="l-text">Bir sinir ağının ileri tarafını (forward pass) ve değerlendirme matematiğini (kayıp) öğrendik. Ama bir kritik soru cevapsız kaldı: <em>kaybı azaltmak için ağırlıkları nasıl güncelleyeceğimizi bilmiyoruz.</em> ML\'de gradyan iniş gördük (<a href="/tutorials/ml-theory/2">ML L2</a>) ama derin ağda gradyanı verimli hesaplamak başlı başına bir matematiktir. <a href="/tutorials/deep/3">Ders 3</a>\'te <strong>backpropagation ve hesap grafları</strong>nı işliyoruz — derin öğrenmenin en güzel matematiksel hilelerinden biri.</p>'
...
+ '<p class="l-text">In <a href="/tutorials/deep/1">Lesson 1</a> we saw a neuron does this: <em>z = w<sup>⊤</sup>x + b</em>, then <em>ŷ = f(z)</em>. If <em>f</em> were just the identity (<em>f(z) = z</em>), the entire network would be a linear transformation. No matter how many layers you stack, the output is still linear.</p>'
...
+ '<p class="l-text">Familiar from logistic regression (<a href="/tutorials/ml-theory/3">ML L3</a>): squashes any real number into (0, 1). For z near 0, σ ≈ 0.5; for large positive, ≈ 1; for large negative, ≈ 0. The default activation in early neural networks (1980-2010).</p>'
...
+ '<p class="l-text">We saw MSE in <a href="/tutorials/ml-theory/2">ML L2</a> and cross-entropy in <a href="/tutorials/ml-theory/3">ML L3</a>. DL builds on the same foundation.</p>'
