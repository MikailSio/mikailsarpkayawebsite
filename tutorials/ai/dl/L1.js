...
+ '<p class="l-text">Klasik makine öğrenmesi (<a href="/tutorials/ai/ml/">ML Theory</a>) dersinde gördüğümüz gibi, bir model girdi vektöründen çıktı tahminini öğrenir. Sinir ağları aynı şeyi yapar — sadece çok daha esnek bir fonksiyon ailesi kullanarak. Bu esneklik onları görüntü, ses, metin gibi karmaşık verilerde son teknoloji yapar.</p>'
...
+ '<p class="l-text">Eğer <em>f(z) = z</em> (özdeşlik) seçersen, bu lineer regresyon — <a href="/tutorials/ai/ml/linear-regression">ML L2</a>. Eğer <em>f(z) = σ(z)</em> (sigmoid) seçersen, lojistik regresyon — <a href="/tutorials/ai/ml/logistic-regression-classification">ML L3</a>. <strong>Yani perceptron derin öğrenmenin değil, klasik ML\'in de temel yapı taşıdır.</strong> Aradaki fark, sinir ağında çok sayıda perceptron\'u üst üste yığmamızdır.</p>'
...
+ '<p class="l-text">XGBoost (<a href="/tutorials/ai/ml/trees-ensembles-xgboost">ML L7</a>) gibi klasik modeller tablo verilerinde mükemmel. Niye DL\'e ihtiyaç var?</p>'
...
+ '<p class="l-text">If <em>f(z) = z</em> (identity), this is linear regression — <a href="/tutorials/ai/ml/linear-regression">ML L2</a>. If <em>f(z) = σ(z)</em> (sigmoid), it is logistic regression — <a href="/tutorials/ai/ml/logistic-regression-classification">ML L3</a>. <strong>So the perceptron is not unique to deep learning — it is the building block of classical ML too.</strong> The difference is that neural networks stack many of them together.</p>'
...
+ '<p class="l-text">Classical models like XGBoost (<a href="/tutorials/ai/ml/trees-ensembles-xgboost">ML L7</a>) are great on tabular data. So why DL?</p>'
...
+ '<p class="l-text">We learned the skeleton of a neural network. But we glossed over one thing: <em>what is an activation function, why do we need it, which to choose?</em> The final step of the forward pass — and packed with nuance. <a href="/tutorials/ai/dl/activations-loss-functions">Lesson 2</a> covers activation functions and the <em>loss functions</em> that pair with them: ReLU, sigmoid, tanh, GELU, Swish, and which to use when.</p>'
