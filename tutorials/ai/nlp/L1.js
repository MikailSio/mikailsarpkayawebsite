...
+ '<li><strong>Duygu analizi (sentiment):</strong> Trendyol, Hepsiburada gibi platformlarda ürün yorumlarının olumlu mu olumsuz mu olduğunu otomatik tespit eder. Müşteri hizmetleri ve ürün yönetimi için kritiktir. Bu kursun <a href="/tutorials/ai/nlp/sentiment-analysis">Ders 4</a>\'ünün ana konusudur.</li>'
+ '<li><strong>Chatbotlar:</strong> Banka, sigorta ve e-ticaret sitelerinde sıkça karşılaşılan otomatik müşteri temsilcileri. Modern örnekler büyük dil modelleri (LLM) kullanır — <a href="/tutorials/ai/nlp/generative-llms">Ders 11</a> ve <a href="/tutorials/ai/nlp/applied-nlp">Ders 12</a>.</li>'
+ '<li><strong>Otomatik özetleme:</strong> Uzun bir haberi üç cümleye, bir tıbbi raporu bir paragrafa indirgeme.</li>'
+ '<li><strong>Adlandırılmış varlık tanıma (NER):</strong> Bir gazete haberinde kişi-yer-kurum isimlerini, bir tıbbi raporda hastalık adlarını otomatik ayıklama. <a href="/tutorials/ai/nlp/sequence-labeling">Ders 7</a>.</li>'
...
+ '<li><strong>Alt-kelime (subword) tokenization:</strong> Kelimeyi istatistiksel olarak öğrenilmiş alt-parçalara böler. Modern büyük dil modellerinin (BERT, GPT) tercih ettiği yaklaşım. <a href="/tutorials/ai/nlp/word-embeddings">Ders 5</a>\'te detayında göreceğiz.</li>'
+ '</ul>'
...
+ '<p class="l-text">Şu anda elinde temiz bir token listesi var: <code>["film", "gerçek", "harika"]</code>. Ama bir makine öğrenmesi modeli kelimeleri değil, <strong>sayıları</strong> ister. <a href="/tutorials/ai/nlp/bow-tfidf-ngrams">Ders 2</a>\'de bu token\'ları nasıl sayılara çevireceğimizi öğreneceğiz: <strong>Bag-of-Words</strong> ve <strong>TF-IDF</strong>. Bunlar klasik NLP\'nin temel taşlarıdır ve hâlâ pek çok pratik problemde kullanılır.</p>'

+ '<p class="l-text">Sonra <a href="/tutorials/ai/nlp/text-classification">Ders 3</a>\'te bu sayıları gerçek bir sınıflandırıcıya bağlayıp ilk NLP modelimizi eğiteceğiz. Sonrasında derin öğrenmeye, Transformer\'lara, BERT ve GPT\'ye geçeceğiz. Ama hepsinin altında bu dersteki adımlar yatar — temiz veri olmadan akıllı model olmaz.</p>'
...
+ '<li><strong>Sentiment analysis:</strong> Platforms like Amazon and IMDB automatically detect whether a review is positive or negative. Critical for customer service at scale. The main subject of <a href="/tutorials/ai/nlp/sentiment-analysis">Lesson 4</a>.</li>'
+ '<li><strong>Chatbots:</strong> Automated customer-service agents on banking, insurance, and e-commerce sites. Modern examples use large language models — see <a href="/tutorials/ai/nlp/generative-llms">Lesson 11</a> and <a href="/tutorials/ai/nlp/applied-nlp">Lesson 12</a>.</li>'
+ '<li><strong>Summarisation:</strong> Reducing a long news article to three sentences, or a clinical report to one paragraph.</li>'
+ '<li><strong>Named Entity Recognition (NER):</strong> Pulling out person/place/organisation names from news, or disease names from clinical notes. <a href="/tutorials/ai/nlp/sequence-labeling">Lesson 7</a>.</li>'
...
+ '<li><strong>Subword tokenization:</strong> Breaks words into statistically learned subword units. The default approach used by modern large language models (BERT, GPT). Covered in <a href="/tutorials/ai/nlp/word-embeddings">Lesson 5</a>.</li>'
+ '</ul>'
...
+ '<p class="l-text">You now have a clean token list: <code>["movi", "truli", "amaz"]</code>. But machine learning models do not consume words; they consume <strong>numbers</strong>. <a href="/tutorials/ai/nlp/bow-tfidf-ngrams">Lesson 2</a> shows how to turn these tokens into numbers using <strong>Bag-of-Words</strong> and <strong>TF-IDF</strong> — the foundations of classical NLP, still widely used in practice.</p>'

+ '<p class="l-text">Then in <a href="/tutorials/ai/nlp/text-classification">Lesson 3</a> we plug those numbers into a real classifier and train our first NLP model. After that we move to deep learning, transformers, BERT and GPT. But everything stands on the steps you learned today — without clean data, no clever model is smart enough.</p>'
