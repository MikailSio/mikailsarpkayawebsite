...
+ '<p class="l-text">Şimdi denetimsiz olarak gizli temaları çıkarabiliyorsun. Ama ya cümle içindeki <em>her kelimeye</em> bir etiket atamak gerekirse — "Mikail" kişi adı, "İstanbul" yer adı gibi? <a href="/tutorials/ai/nlp/sequence-labeling">Ders 7</a>\'de bu görevi öğreniyoruz: <strong>dizi etiketleme</strong> ve <strong>NER (Named Entity Recognition)</strong>. Konu modelleme döküman seviyesindeydi; NER kelime seviyesinde bir denetimli görev.</p>'
...
+ '<p class="l-text">You can now extract latent themes without any labels. But what if we need a label on <em>each word</em> in a sentence — "Mikail" as a person, "Istanbul" as a location? <a href="/tutorials/ai/nlp/sequence-labeling">Lesson 7</a> covers this task: <strong>sequence labelling</strong> and <strong>NER (Named Entity Recognition)</strong>. Topic modelling worked at the document level; NER is a supervised, word-level task.</p>'
