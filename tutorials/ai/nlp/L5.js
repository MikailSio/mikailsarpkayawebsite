...
+ '<p class="l-text">This is severe for Turkish. The agglutinative structure (recall <a href="/tutorials/ai/nlp/text-preprocessing">Lesson 1</a>) generates hundreds of inflected forms from a single root: <em>evlerimizdekilerden, evimizdekilerden, evimdekilerden</em>. Word2Vec sees these as different words and needs a separate vector for each.</p>'
...
+ '<p class="l-text">Every word now has a vector that reflects its meaning. But it is still a static vector — "amazing" is represented the same way in every sentence, even though the meaning can shift with context ("amazing day" vs. "amazing, late again"). <a href="/tutorials/ai/nlp/topic-modeling">Lesson 6</a> tackles topic modelling — finding latent themes across documents — and starting from <a href="/tutorials/ai/nlp/seq2seq-attention">Lesson 9</a> we move to contextual embeddings (RNN and Transformer outputs).</p>'
