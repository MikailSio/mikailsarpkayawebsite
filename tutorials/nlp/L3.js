...
+ '<p class="l-text">Bizim girdimiz <a href="/tutorials/nlp/2">Ders 2</a>\'den geliyor: bir film yorumunun TF-IDF vektörü. Çıktımız iki sınıftan biri olacak. Mini corpus\'umuzu hatırlayalım:</p>'
...
+ '<p class="l-text">Artık temizlenmiş, sayılaştırılmış ve sınıflandırılmış metinlerle çalışabiliyorsun. Naive Bayes ve lojistik regresyon klasik NLP\'nin temel taşlarıdır — pek çok pratik problemde hâlâ ilk denenen modeller. <a href="/tutorials/nlp/4">Ders 4</a>\'te bu yöntemleri en yaygın kullanım alanlarından birine — <strong>duygu analizi</strong>na — uygulayacağız. Türkçe yorum verisi üzerinde tam bir uçtan uca pipeline kuracağız: ön işleme, vektörleştirme, sınıflandırma, değerlendirme.</p>'
...
+ '<p class="l-text">Our input comes from <a href="/tutorials/nlp/2">Lesson 2</a>: a TF-IDF vector of a movie review. Our output is one of two classes. Recall the mini corpus:</p>'
...
+ '<p class="l-text">You can now work with cleaned, vectorized, and classified text. Naive Bayes and logistic regression are foundations of classical NLP — they are still the first models tried for many practical problems. <a href="/tutorials/nlp/4">Lesson 4</a> applies these methods to one of the most popular use cases — <strong>sentiment analysis</strong>. We will build an end-to-end pipeline on Turkish review data: preprocessing, vectorization, classification, evaluation.</p>'
