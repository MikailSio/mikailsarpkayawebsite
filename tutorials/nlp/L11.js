...
+ '<p class="l-text"><strong>Büyük dil modeli (LLM)</strong>, devasa metin külliyatları üzerinde önceden eğitilmiş, milyarlarca parametreli Transformer\'lardır. Mimari olarak <a href="/tutorials/nlp/10">Ders 10</a>\'da gördüğümüz GPT\'nin aynısı — sadece çok daha büyük, çok daha çeşitli veri üzerinde, çok daha uzun süre eğitilmiş.</p>'
...
+ '<p class="l-text">Büyük internet metni üzerinde sonraki kelimeyi tahmin etme görevi (<a href="/tutorials/nlp/8">Ders 8</a>). Trilyonlarca token görür, dilin yapısını ve dünya bilgisinin önemli bir kısmını öğrenir. Ama henüz "talimat takip etmeyi" bilmiyor — sadece metin tamamlamayı yapıyor.</p>'
...
+ '<li><strong>Pratik öneri:</strong> Çoğu görev için en kısa yol — bir genel-amaçlı LLM (GPT-4, Claude) + iyi prompt + gerektiğinde RAG (<a href="/tutorials/nlp/12">Ders 12</a>). Tam yerel kontrol gerekiyorsa Llama-3 veya Mistral fine-tune edilir.</li>'
+ '</ul>'
...
+ '<li><strong>DeepSeek-R1:</strong> V3 üzerine inşa edilen muhakeme varyantı (Ocak 2025) — <a href="/tutorials/nlp/14">Ders 14</a>\'te o ele alınacak.</li>'
+ '</ul>'
...
+ '<li><strong>Eski bilgi:</strong> Eğitim kesimi vardır. 2026 olayları 2024 modelinde olmaz. RAG (<a href="/tutorials/nlp/12">Ders 12</a>) bunu çözer.</li>'
...
+ '<p class="l-text">LLM\'in mimari ve eğitim hikayesini öğrendin. Şimdi bunlar gerçek üretim sistemlerinde nasıl kullanılıyor — tek bir prompt çağrısının çok ötesinde — onu göreceksin. <a href="/tutorials/nlp/12">Ders 12</a>: <strong>RAG, ajanlar, çok dilli NLP, etik ve uçtan uca proje şablonu</strong>. Bu kursun final dersi, öğrendiğin her şeyi gerçek uygulamada birleştirir.</p>'
...
+ '<div class="calc-highlight"><strong>What you will learn:</strong> In <a href="/tutorials/nlp/10">Lesson 10</a> we covered the Transformer and the BERT/GPT base models. Now we move to <strong>large language models (LLMs)</strong>: GPT-3, GPT-4, Claude, Llama — models with hundreds of billions of parameters. How scale produced quality (scaling laws), how instruction-tuning and RLHF aligned them with human preferences, how LoRA enables low-resource fine-tuning, what prompt engineering is, and where emergent capabilities hit their limits.</div>'
...
+ '<p class="l-text">A <strong>large language model (LLM)</strong> is a Transformer with billions of parameters, pretrained on enormous text corpora. Architecturally identical to the GPT we saw in <a href="/tutorials/nlp/10">Lesson 10</a> — just much bigger, trained on much more diverse data, for much longer.</p>'
...
+ '<p class="l-text">Next-token prediction on huge web text (<a href="/tutorials/nlp/8">Lesson 8</a>). Trillions of tokens; the model learns language structure and a large slice of world knowledge. But it does not yet "follow instructions" — it just continues text.</p>'
...
+ '<li><strong>Practical recommendation:</strong> for most tasks, the shortest path is a general LLM (GPT-4, Claude) + good prompts + RAG when needed (<a href="/tutorials/nlp/12">Lesson 12</a>). For full local control, fine-tune Llama-3 or Mistral.</li>'
+ '</ul>'
...
+ '<li><strong>DeepSeek-R1:</strong> reasoning variant built on V3 (Jan 2025) — covered in <a href="/tutorials/nlp/14">Lesson 14</a>.</li>'
+ '</ul>'
...
+ '<li><strong>Stale knowledge:</strong> training has a cutoff. Events after 2024 do not exist for a 2024-trained model. RAG (<a href="/tutorials/nlp/12">Lesson 12</a>) fixes this.</li>'
...
+ '<p class="l-text">You now know the LLM architecture and training story. Next we look at how they are used in real production systems — far beyond a single prompt call. <a href="/tutorials/nlp/12">Lesson 12</a>: <strong>RAG, agents, multilingual NLP, ethics, and an end-to-end project template</strong>. The final lesson of this course brings everything together for real applications.</p>'
