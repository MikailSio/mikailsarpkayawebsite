...
+ '<p class="l-text"><a href="/tutorials/nlp/3">Ders 3</a>\'te öğrendiğimiz Naive Bayes ve lojistik regresyon. TF-IDF + bigram + lojistik regresyon kombinasyonu pek çok Türkçe sentiment veri setinde %85-90 doğruluk verir. Pratik, hızlı, üretimde sıkça kullanılır.</p>'

+ '<h3 class="l-subtitle">Yöntem 3: Derin öğrenme (BERT türevleri)</h3>'

+ '<p class="l-text">Türkçe için <strong>BERTurk</strong> (dbmdz tarafından eğitilen Türkçe BERT) ince ayar (fine-tuning) ile %92-95 doğruluk verir. Daha pahalı ama olumsuzlama, ironi ve uzun bağlam konularında klasik yöntemlerden açık ara önde. <a href="/tutorials/nlp/10">Ders 10</a> ve <a href="/tutorials/nlp/11">Ders 11</a>\'de detaylanır.</p>'
...
+ '<p class="l-text">The Naive Bayes and logistic regression we learned in <a href="/tutorials/nlp/3">Lesson 3</a>. The combination of TF-IDF + bigrams + logistic regression hits 85-90% on most Turkish sentiment datasets. Practical, fast, and still common in production.</p>'

+ '<h3 class="l-subtitle">Approach 3: deep learning (BERT family)</h3>'

+ '<p class="l-text">For Turkish, fine-tuning <strong>BERTurk</strong> (the Turkish BERT trained by dbmdz) gives 92-95% accuracy. More expensive but clearly ahead of classical methods on negation, irony, and long context. See <a href="/tutorials/nlp/10">Lesson 10</a> and <a href="/tutorials/nlp/11">Lesson 11</a>.</p>'
...
+ '<p class="l-text">ABSA is two tasks: (1) extract aspects from the sentence, (2) classify sentiment per aspect. Sequence labelling methods from <a href="/tutorials/nlp/7">Lesson 7</a> (BIO tagging) are used for aspect extraction. Modern approaches do both jointly with a single BERT-based model.</p>'
...
+ '<p class="l-text">Sentiment is one of classical NLP\'s most mature and most-used tasks. Everything we have done relies on the assumption that "a word is just a feature". But "amazing" and "wonderful" are close in meaning — BoW does not know this. <a href="/tutorials/nlp/5">Lesson 5</a> closes that gap with <strong>word embeddings</strong>. Word2Vec, GloVe, FastText — methods that turn words into dense vectors that capture meaning.</p>'
