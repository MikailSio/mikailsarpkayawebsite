...
+ '<p class="l-text">Hiç görülmemiş bigram\'lara bile küçük ama sıfır olmayan olasılık verir. Naive Bayes\'te de gördük (<a href="/tutorials/nlp/3">Ders 3</a>).</p>'
...
+ '<p class="l-text">"Bu film gerçekten harikaydı" gördüyse, "Bu dizi gerçekten harikaydı"yı tahmin edebilir mi? <strong>Hayır</strong>. n-gram için "film" ve "dizi" tamamen ayrı kelimeler, hiçbir benzerlik yok. Oysa <a href="/tutorials/nlp/5">Ders 5</a>\'teki embedding\'ler bu iki kelimeyi yakın vektörlere koyardı.</p>'
...
+ '<li><strong>Uzun bağlam:</strong> Sinir ağı (özellikle RNN — <a href="/tutorials/nlp/9">Ders 9</a>) çok daha uzun bağlamı taşıyabilir.</li>'
+ '</ul>'
...
+ '<li><strong>2010 — RNN LM:</strong> Tekrarlayan ağlarla değişken uzunluklu bağlam (<a href="/tutorials/nlp/9">Ders 9</a>).</li>'
+ '<li><strong>2014 — LSTM LM:</strong> Uzun bağımlılıkları daha iyi yakalayan RNN varyantı.</li>'
+ '<li><strong>2017 — Transformer LM:</strong> Self-attention ile paralelleştirilebilir, çok uzun bağlam (<a href="/tutorials/nlp/10">Ders 10</a>).</li>'
+ '<li><strong>2018+ — GPT, BERT:</strong> Büyük corpus üzerinde önceden eğitilmiş Transformer\'lar (<a href="/tutorials/nlp/11">Ders 11</a>).</li>'
+ '</ol>'
...
+ '<p class="l-text">Dil modeli kavramı modern NLP\'nin merkezindedir. n-gram\'larla görmediğin dezavantajları sinirsel modeller çözer. <a href="/tutorials/nlp/9">Ders 9</a>\'da <strong>RNN ve LSTM</strong>\'i göreceğiz — değişken uzunluklu bağlam taşıyan ilk başarılı sinirsel mimariler. Sonra <a href="/tutorials/nlp/10">Ders 10</a>\'da Transformer\'larla tanışacağız. Bu derste kurduğun temel — <em>sıradaki kelimeyi tahmin etme</em> — modern her LLM\'in özünde aynı kalır.</p>'
...
+ '<p class="l-text">It assigns a small but non-zero probability even to never-seen bigrams. We saw the same trick in Naive Bayes (<a href="/tutorials/nlp/3">Lesson 3</a>).</p>'
...
+ '<p class="l-text">If the model has seen "the movie was truly amazing", can it predict "the show was truly amazing"? <strong>No</strong>. For n-grams "movie" and "show" are entirely separate words with no similarity. Embeddings (<a href="/tutorials/nlp/5">Lesson 5</a>) on the other hand would place these close.</p>'
...
+ '<li><strong>Long context:</strong> a neural network (especially RNN — <a href="/tutorials/nlp/9">Lesson 9</a>) can carry much longer context.</li>'
+ '</ul>'
...
+ '<li><strong>2010 — RNN LM:</strong> recurrent networks with variable-length context (<a href="/tutorials/nlp/9">Lesson 9</a>).</li>'
+ '<li><strong>2014 — LSTM LM:</strong> RNN variant that captures long dependencies better.</li>'
+ '<li><strong>2017 — Transformer LM:</strong> self-attention; parallelisable, very long context (<a href="/tutorials/nlp/10">Lesson 10</a>).</li>'
+ '<li><strong>2018+ — GPT, BERT:</strong> Transformers pretrained on massive corpora (<a href="/tutorials/nlp/11">Lesson 11</a>).</li>'
+ '</ol>'
...
+ '<p class="l-text">Language modelling sits at the heart of modern NLP. The shortcomings of n-grams are exactly what neural models solve. <a href="/tutorials/nlp/9">Lesson 9</a> covers <strong>RNN and LSTM</strong> — the first successful neural architectures for variable-length context. Then <a href="/tutorials/nlp/10">Lesson 10</a> introduces Transformers. The core idea you learned here — <em>predict the next word</em> — stays the same in every modern LLM.</p>'
